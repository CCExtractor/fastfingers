...
const gchar *
ff_key_get_text (FFKey *key)
{
  g_return_val_if_fail (FF_IS_KEY (key), NULL);

  return gtk_label_get_text(GTK_LABEL (key));
}

void
ff_key_set_text (FFKey *key,
		 const gchar* text)
{
  ...
  gtk_label_set_text(GTK_LABEL (key), text);
}
...
static void
ff_key_set_property (GObject *object,
		     guint prop_id,
		     const GValue *value,
		     GParamSpec *pspec)
{
...
static void
ff_key_get_property (GObject *object,
		     guint prop_id,
		     GValue *value,
		     GParamSpec *pspec)
{
...
static void
ff_key_class_init (FFKeyClass *klass)
{
  ...
  gtk_widget_class_set_template_from_resource (widget_class, "/org/ccextractor/FastFingers/ui/key.ui");
  gtk_widget_class_set_css_name (widget_class, "key");
}

static void
ff_key_init (FFKey *self)
{
...
{
  FFKey *_key = g_object_new (FF_TYPE_KEY,
			      "text", text,
			      NULL);
