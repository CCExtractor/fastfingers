...
void ff_application_page_init(GtkStack *stack, const char *title) {
    ...
    if (learned_of_all < 20) {
        ...
        char *placeHolder;
        if (learned_of_all == 19)
            placeHolder = g_strdup_printf("You need to learn one more shortcut to enable quiz mode");
        else
            placeHolder = g_strdup_printf("You need to learn %d more shortcuts to enable quiz mode",
                                          20 - learned_of_all);
