...
static void ff_buttonbox_dispose(GObject *object) {
...
static void ff_buttonbox_set_property(GObject *object, guint prop_id,
...
static void ff_buttonbox_get_property(GObject *object, guint prop_id,
...
static void ff_buttonbox_class_init(FFButtonboxClass *klass) {
  ...
  props[PROP_BACK_LABEL] = g_param_spec_string(
      "back-label", "Back-Label", "Label of the back button", "Back",
      G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  props[PROP_BACK_VISIBLE] = g_param_spec_boolean(
      "back-visible", "Back-Visible", "Visibility of the back button", 0,
      G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  props[PROP_SETTINGS_VISIBLE] =
      g_param_spec_boolean("settings-visible", "Settings-Visible",
                           "Visibility of the settings button", 1,
                           G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  g_object_class_install_properties(object_class, LAST_PROP, props);

  gtk_widget_class_set_template_from_resource(
      widget_class, "/org/ccextractor/FastFingers/ui/button-box.ui");
...
void back_click_cb(GtkWidget *widget, GdkEvent *event, gpointer user_data) {
  ff_switch_previous();
}
...
void settings_show_cb(GtkWidget *widget, FFButtonbox *buttonbox) {
  if (!ff_buttonbox_get_settings_visible(buttonbox))
    gtk_widget_set_visible(widget, FALSE);
}

static void ff_buttonbox_init(FFButtonbox *self) {
  gtk_widget_init_template(GTK_WIDGET(self));

  g_signal_connect(GTK_BUTTON(self->back), "clicked", G_CALLBACK(back_click_cb),
                   NULL);
  g_signal_connect(GTK_BUTTON(self->back), "show", G_CALLBACK(back_show_cb),
                   self);
  g_signal_connect(GTK_BUTTON(self->settings), "clicked",
                   G_CALLBACK(settings_click_cb), NULL);
  g_signal_connect(GTK_BUTTON(self->settings), "show",
                   G_CALLBACK(settings_show_cb), self);
}

GtkWidget *ff_buttonbox_new(void) {
  GtkWidget *_buttonbox = g_object_new(FF_TYPE_BUTTONBOX, NULL);
  return _buttonbox;
}
