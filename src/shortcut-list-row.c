...
static void
ff_shortcut_list_row_set_property (GObject *object,
				   guint prop_id,
				   const GValue *value,
				   GParamSpec *pspec)
{
...
static void
ff_shortcut_list_row_get_property (GObject *object,
				   guint prop_id,
				   GValue *value,
				   GParamSpec *pspec)
{
...
static void
ff_shortcut_list_row_class_init (FFShortcutListRowClass *klass)
{
  ...
  props[PROP_LEFT_TEXT] =
    g_param_spec_string ("left-text",
			 "left-text",
			 "Left text of the row",
			 "unknown category",
  ...
  props[PROP_RIGHT_TEXT] =
    g_param_spec_string ("right-text",
			 "right-text",
			 "Right text of the row",
			 "0/0",
			 G_PARAM_READWRITE|G_PARAM_STATIC_STRINGS);

  g_object_class_install_properties (object_class, LAST_PROP, props);

  gtk_widget_class_set_template_from_resource (widget_class, "/org/ccextractor/FastFingers/ui/shortcut-list-row.ui");
  ...
  gtk_widget_class_bind_template_child (widget_class, FFShortcutListRow, left_label);
  gtk_widget_class_bind_template_child (widget_class, FFShortcutListRow, right_label);
}

static void
ff_shortcut_list_row_init (FFShortcutListRow *self)
{
...
{
  GtkWidget *_row = g_object_new (FF_TYPE_SHORTCUT_LIST_ROW,
				  "left-text", left,
				  "right-text", right,
				  NULL);
  return _row;
