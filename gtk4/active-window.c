...
char *get_active_from_xorg (void)
{
  ...
  status = XGetWindowProperty(display, root_window, filter_atom, 0, 1024, False, AnyPropertyType,
  ...
  status = XGetWindowProperty(display, active_window, filter_atom, 0, 1024, False, AnyPropertyType,
			      &actual_type, &actual_format, &nitems, &bytes_after, &prop);
...
char *get_active_from_gnome_shell (void)
{
  ...
  GVariant *result = g_dbus_connection_call_sync (c,
						  "org.gnome.Shell",
						  "/org/gnome/Shell",
						  "org.gnome.Shell",
						  "Eval",
						  g_variant_new ("(s)", "global.get_window_actors().map(a=>a.meta_window).find(w=>w.has_focus()).get_wm_class()"),
						  NULL,
