...
enum { PROP_0, PROP_TITLE, PROP_PROGRESS, LAST_PROP };
...
static void ff_card_init(FFCard *self) {
  ...
  g_signal_connect(self->gest_click, "pressed", G_CALLBACK(card_click_cb),
                   self);
}

GtkWidget *ff_card_new(const gchar *title, double progress) {
  GtkWidget *_card =
      g_object_new(FF_TYPE_CARD, "title", title, "progress", progress, NULL);
  return _card;
}

static void card_click_cb(GtkGestureClick *gesture, gint n_press, gdouble x,
                          gdouble y, GtkWidget *card) {
